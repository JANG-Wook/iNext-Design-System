...
- **Data Visual(Chart)** — `body-sm` 의 `$description` 이 *"Data Visual 축 레이블"* 을 용례로 적고 있고,
  미결 21(`tnum`)도 *"Chart·Table 착수 시 만든다"* 고 적었다. **P0~P3 어디에도 없다.**
  넣을지 빼야 할지 정해야 한다. 빼면 위 두 곳의 문구를 함께 고친다.
- **개수 불일치** — 위 합계는 **27종**인데 미결 9 는 *"계획된 컴포넌트 29종"* 이라고 적고 있다.
  29의 출처를 확인하지 못했다. 이 목록을 정본으로 삼는다면 미결 9 의 숫자를 고친다.
